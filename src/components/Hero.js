  ...
  return (
    <section className="relative max-w-[1024px] mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-y-20 py-16 md:py-32 lg:py-40 min-h-screen items-center" id="home">
      ...
      <div className="flex flex-col gap-6 mt-12 md:mt-0">
        <h1 className="text-4xl md:text-[3.5rem] font-bold leading-tight text-slate-900">
          Olá, sou <span className="text-slate-700">Thiago</span> <br />
          estudante de engenharia de produção e desenvolvimento de software.
        </h1>

        {/* Link do Next.js substituindo a tag <a> */}
        <Link href="#contato" className="inline-block w-max bg-slate-900 text-white px-10 py-3 font-semibold rounded-md hover:shadow-[0_10px_36px_rgba(0,0,0,0.15)] hover:-translate-y-1 transition-all duration-300">
          Contato
        </Link>
        {/* NOVO BOTÃO: Leva direto para a rota /dashboard */}
        <Link href="/dashboard" className="inline-block w-max bg-slate-700 text-white px-8 py-3 font-semibold rounded-md hover:shadow-[0_10px_36px_rgba(0,0,0,0.15)] hover:-translate-y-1 transition-all duration-300 flex items-center gap-2">
          <i className='bx bx-desktop text-xl'></i> Ver Sistema PCP
        </Link>

        <div className="flex flex-row gap-4 mt-8">
          <a href="https://www.linkedin.com/in/thiago-trindade-351892208/" target="_blank" rel="noopener noreferrer" className="text-2xl text-slate-900 hover:text-slate-600 transition-colors">
            <i className='bx bxl-linkedin'></i>
          </a>
          <a href="https://github.com/thiagotrindade" target="_blank" rel="noopener noreferrer" className="text-2xl text-slate-900 hover:text-slate-600 transition-colors">
